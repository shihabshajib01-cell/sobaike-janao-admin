...
const FALLBACK_AUDIT_LOGS: AuditLogItem[] = [
  ...
  },
  {
    ...
    details: {
      added_permissions: ['complaints.verify', 'complaints.unpublish'],
      removed_permissions: [],
      previous_count: 6,
      new_count: 8,
    },
  ...
  },
  {
    ...
    details: {
      target_user_name: 'Tanvir Ahmed',
      previous_role_id: 'field_officer',
      previous_role_name: 'Departmental Field Officer',
      new_role_id: 'content_moderator',
      new_role_name: 'Content Moderator',
    },
  ...
  },
  {
    ...
    details: {
      ...
      role_name: 'Departmental Field Officer',
      active: false,
    },
  ...
  },
  {
    ...
    target_id: 'senior_investigator',
    details: {
      name: 'Senior Investigative Officer',
      active: true,
    },
  ...
  async getAuditLogs(params: AuditLogQueryParams = {}): Promise<AuditLogListResponse> {
    ...
    if (status === 'configured') {
      try {
        ...
        // Response handling
        const rawLogs: any[] = Array.isArray(data) ? data : data?.logs || [];
        ...
        const mapped: AuditLogItem[] = rawLogs.map((item: any) => ({
          ...
          actor_display_name: item.actor_display_name || item.actor_name || null,
          action: item.action,
