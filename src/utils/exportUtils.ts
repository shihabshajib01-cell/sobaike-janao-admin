...
export function exportAuditLogsToPdf(
  ...
  try {
    const doc = new jsPDF({ orientation: 'landscape', unit: 'mm', format: 'a4' });
    const filename = customFilename || `sobaike_audit_logs_${new Date().toISOString().slice(0, 10)}.pdf`;
    ...
    if (filterSummary?.action && filterSummary.action !== 'all') {
      filterItems.push({ label: 'Action', value: filterSummary.action });
    }
    if (filterSummary?.search) {
      filterItems.push({ label: 'Search', value: `"${filterSummary.search}"` });
    }
    ...
    const startY = setupPdfHeaderAndFooter(
      ...
      'System-wide record of operator actions, status transitions, and data modifications.',
      filterItems,
      logs.length,
      true
    );
    ...
    const headers = [
      ['Log ID', 'Timestamp', 'Actor / Operator', 'Role', 'Module', 'Action', 'Entity', 'Details']
    ];

    const body = logs.map((l) => [
      l.id,
      l.timestamp ? new Date(l.timestamp).toLocaleString() : '-',
      `${sanitizeText(l.actor?.name)}\n(${sanitizeText(l.actor?.email)})`,
      sanitizeText(l.actor?.role, 'Staff'),
    ...
    console.error('Failed to generate Audit Logs PDF:', error);
    return false;
  }
}

/* ==========================================================================
   4. MAP COMPLAINTS EXPORT (CSV & PDF)
   ========================================================================== */
...
/* ==========================================================================
   5. ANALYTICS REPORT EXPORT (CSV & PDF)
   ========================================================================== */

export function exportAnalyticsToCsv(data: AnalyticsDataResponse, customFilename?: string): boolean {
  const filename = customFilename || `sobaike_analytics_report_${new Date().toISOString().slice(0, 10)}.csv`;
  const headers = ['Report Section', 'Metric / Item Name', 'Subcategory / Dimension', 'Count / Value', 'Percentage / Rate'];
  const rows: (string | number | boolean | null | undefined)[][] = [];

  if (data.summary) {
    rows.push(['KPI Summary', 'Total Complaints', '-', data.summary.totalComplaints, '100%']);
    rows.push(['KPI Summary', 'Published to Public Feed', '-', data.summary.published, `${((data.summary.published / (data.summary.totalComplaints || 1)) * 100).toFixed(1)}%`]);
    rows.push(['KPI Summary', 'Resolved Cases', '-', data.summary.resolved, `${((data.summary.resolved / (data.summary.totalComplaints || 1)) * 100).toFixed(1)}%`]);
    rows.push(['KPI Summary', 'Official Responses Issued', '-', data.summary.responses, '-']);
    rows.push(['KPI Summary', 'Active Categories Monitored', '-', data.summary.activeCategories, '-']);
  }

  if (data.statusDistribution && data.statusDistribution.length > 0) {
    data.statusDistribution.forEach((s) => {
      rows.push(['Status Breakdown', s.labelEn, s.status, s.count, `${s.percentage}%`]);
    });
  }

  if (data.categoryDistribution && data.categoryDistribution.length > 0) {
    data.categoryDistribution.forEach((c) => {
      rows.push(['Category Breakdown', c.nameEn, `Resolved: ${c.resolvedCount}`, c.count, `${c.percentage}%`]);
      if (c.subcategories) {
        c.subcategories.forEach((sub) => {
          rows.push(['Subcategory Detail', c.nameEn, sub.nameEn, sub.count, `${sub.percentage}%`]);
        });
      }
    });
  }

  if (data.locationDistribution && data.locationDistribution.length > 0) {
    data.locationDistribution.forEach((loc) => {
      rows.push(['Location Insights', loc.location, loc.area || '-', loc.count, `${loc.percentage}%`]);
    });
  }

  if (data.trends && data.trends.length > 0) {
    data.trends.forEach((t) => {
      rows.push(['Daily Trend', t.date, `Resolved: ${t.resolvedCount} | Responses: ${t.responsesCount}`, t.complaintsCount, '-']);
    });
  }
...
export function exportAnalyticsToPdf(
  ...
  try {
    ...
    const filename = customFilename || `sobaike_analytics_summary_${new Date().toISOString().slice(0, 10)}.pdf`;

    const filterItems: { label: string; value: string }[] = [];
    if (filterSummary?.dateRange) {
      filterItems.push({ label: 'Period', value: filterSummary.dateRange.toUpperCase() });
    }
    if (filterSummary?.categoryId && filterSummary.categoryId !== 'all') {
    ...
    const startY = setupPdfHeaderAndFooter(
      doc,
      'Civic Operations & Analytics Executive Summary',
      'Comprehensive performance report covering resolution rates, ward distribution, and category breakdowns.',
      filterItems,
    ...
    const kpiHeaders = [['Metric Indicator', 'Volume / Count', 'Operational Share / Rate']];
    const kpiBody = [
      ['Total Citizen Complaints', String(data.summary?.totalComplaints || 0), '100%'],
      ['Published to Public Feed', String(data.summary?.published || 0), `${(((data.summary?.published || 0) / (data.summary?.totalComplaints || 1)) * 100).toFixed(1)}%`],
      ['Resolved & Remediated Cases', String(data.summary?.resolved || 0), `${(((data.summary?.resolved || 0) / (data.summary?.totalComplaints || 1)) * 100).toFixed(1)}%`],
      ['Official Responses Issued', String(data.summary?.responses || 0), 'Verified Statements'],
      ['Active Civic Categories', String(data.summary?.activeCategories || 0), 'Monitored Domains'],
    ];
    ...
    if (data.categoryDistribution && data.categoryDistribution.length > 0) {
      ...
      doc.text('CATEGORY & REMEDIATION BREAKDOWN', 14, finalY);

      const catHeaders = [['Category Name', 'Total Cases', 'Resolved', 'Share (%)']];
    ...
    if (data.statusDistribution && data.statusDistribution.length > 0) {
      ...
      doc.setFontSize(10);
      doc.setTextColor(15, 23, 42);
      doc.text('LIFECYCLE STATUS DISTRIBUTION', 14, finalY2);

      const statusHeaders = [['Lifecycle Stage', 'Count', 'Distribution (%)']];
      const statusBody = data.statusDistribution.map((s) => [
        sanitizeText(s.labelEn),
        String(s.count),
        `${s.percentage}%`,
      ]);
...
/* ==========================================================================
   6. DASHBOARD OVERVIEW EXPORT (CSV & PDF)
   ========================================================================== */
