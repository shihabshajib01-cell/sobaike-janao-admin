...
export const CategoryPopularitySummary: React.FC = () => {
  ...
  const load = async () => {
    ...
    setError(false);
    try {
      const [ranking, segments] = await Promise.all([
        getCategoryPopularity(),
  ...
  const rows = useMemo(() => metrics.slice().sort((a, b) => a.popularityRank - b.popularityRank), [metrics]);
              ...
              <tbody>
                {loading && rows.length === 0 ? (
                  ...
                  rows.map((item) => {
                    ...
                    return (
                      <tr key={item.segmentId} className="border-b border-slate-100 last:border-b-0 dark:border-slate-800/70">
                        ...
                        <td className="py-3 pr-3 text-slate-600 dark:text-slate-300">
                          <span className="inline-flex items-center gap-1.5"><FileText className="h-3.5 w-3.5" aria-hidden="true" />{formatNumber(item.publishedPostCount)}</span>
                        </td>
                        <td className="py-3 pr-3 text-slate-600 dark:text-slate-300">
                          <span className="inline-flex items-center gap-1.5"><Eye className="h-3.5 w-3.5" aria-hidden="true" />{formatNumber(item.viewCount)}</span>
                        </td>
                        <td className="py-3 text-slate-600 dark:text-slate-300">
                          <span className="inline-flex items-center gap-1.5"><Share2 className="h-3.5 w-3.5" aria-hidden="true" />{formatNumber(item.shareCount)}</span>
                        </td>
                      </tr>
                    );
                  })
