-- ==============================================================================
-- SOBAIKE JANAO ADMIN — RBAC CORRECTION PASS & EVIDENCE HARDENING (PHASE 3F)
-- ==============================================================================
-- Repository: shihabshajib01-cell/sobaike-janao-admin
-- Target: Supabase PostgreSQL Database (sobaike-production)
-- Purpose:
--   1. Evidence Storage Security Hardening:
--      - Ensure 'complaint-evidence' bucket has public = false in storage.buckets.
--      - Revoke any public or anon read policies on 'complaint-evidence'.
--      - Enforce SELECT RLS on storage.objects requiring active admin + 'complaints.evidence_view'.
--   2. Moderation RPC Overload & Status Transition Hardening:
--      - Drop any ambiguous 1-parameter signature for admin_unpublish_complaint(text).
--      - Verify admin_publish_complaint:
--        * Allowed ONLY from 'submitted' or 'unpublished' -> 'published'
--        * Locks complaint row FOR UPDATE
--        * Inserts into public.complaint_updates (preserves timeline)
--        * Inserts into public.admin_audit_logs
--      - Verify admin_unpublish_complaint:
--        * Allowed ONLY from 'published' -> 'unpublished'
--        * Locks complaint row FOR UPDATE
--        * Inserts into public.complaint_updates
--        * Inserts into public.admin_audit_logs
--      - Verify admin_reject_complaint:
...
-- ------------------------------------------------------------------------------
-- 1. HARDEN PRIVATE EVIDENCE STORAGE BUCKET & RLS
-- ------------------------------------------------------------------------------
DO $$
BEGIN
    -- 1.1. Ensure bucket is not public
    IF EXISTS (
    ...
    -- 1.2. Ensure storage.objects has clean, restrictive policy for 'complaint-evidence'
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'storage' AND table_name = 'objects'
    ) THEN
        -- Drop any potential conflicting or duplicate policies
        DROP POLICY IF EXISTS "complaint_evidence_storage_view_policy" ON storage.objects;
        DROP POLICY IF EXISTS "Public complaint-evidence access" ON storage.objects;
        DROP POLICY IF EXISTS "Allow anon read complaint evidence" ON storage.objects;
        
        -- Create strict authoritative policy:
        -- Authenticated callers ONLY, must be active admin, must have complaints.evidence_view
...
-- ------------------------------------------------------------------------------
-- 2. RESOLVE MODERATION RPC OVERLOADS
-- ------------------------------------------------------------------------------
-- Drop any legacy 1-arg overload of admin_unpublish_complaint to prevent PostgREST ambiguity
DROP FUNCTION IF EXISTS public.admin_unpublish_complaint(text);

-- ------------------------------------------------------------------------------
-- 3. HARDENED MODERATION RPC: admin_publish_complaint(p_complaint_id)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 4. HARDENED MODERATION RPC: admin_unpublish_complaint(p_complaint_id, p_reason)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 5. HARDENED MODERATION RPC: admin_reject_complaint(p_complaint_id, p_reason_code, p_note)
-- ------------------------------------------------------------------------------
