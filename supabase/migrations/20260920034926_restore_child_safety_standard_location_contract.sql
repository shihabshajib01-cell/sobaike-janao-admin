-- Restore the new child-safety subtype to the shared incident-location contract.
-- Keep its separate identity/evidence privacy hardening, but do not replace the
-- existing report system's Division -> District -> Thana/Upazila -> optional
-- detailed-address location flow with a category-specific reduced location model.
