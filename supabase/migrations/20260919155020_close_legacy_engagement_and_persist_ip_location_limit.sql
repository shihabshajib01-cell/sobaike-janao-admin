-- Close legacy anonymous engagement RPC bypass and extend the authoritative
-- Edge write limiter to coarse IP-location lookups.
...
begin
  ...
  if v_action='ip_location' and (
       v_minute_count >= 120 or v_hour_count >= 1200 or v_day_count >= 10000
     ) then
    return jsonb_build_object('allowed',false,'retryAfterSeconds',60);
  end if;
