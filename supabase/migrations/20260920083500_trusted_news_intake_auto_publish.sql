...
begin
  ...
  loop
    ...
    -- For an approved publisher, an omitted field means the source did not
    -- state it. Validate supplied values, but never invent a missing answer.
    if v_field.required and not v_trusted_source then
      if v_value is null
    ...
    if v_field.field_type in ('text','textarea','phone','email','url','date','time','month','select','radio') then
      ...
      if v_field.field_type='email'
         ...
         and v_text !~* '^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
begin
  ...
  if v_missing_checkbox is not null
     and not (
       lower(coalesce(new.custom_field_answers->>'trustedSourceAuto','false'))='true'
       and coalesce(new.custom_field_answers->>'sourceTruthMode','')='approved_publisher'
     ) then
...
GRANT EXECUTE ON FUNCTION public.process_trusted_news_intake_candidate(jsonb) TO service_role;
 then
    ...
    elsif v_field.field_type in ('select','radio')
       and jsonb_array_length(v_field.options)>0
       and not exists (
         select 1 from jsonb_array_elements(v_field.options) option_row
