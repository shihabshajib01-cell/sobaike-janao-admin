-- Align the sanitized Public report RPCs with the existing Public ReportItem contract.
-- utility_end_time is already persisted on complaints for load-shedding/gas-shortage reports.
-- This migration only exposes that existing field through the same sanitized public payloads.
-- No reporter/device location, private contact, RLS, grants, or submission behavior is changed.
