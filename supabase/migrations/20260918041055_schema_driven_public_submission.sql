-- Schema-driven public submission contract + configured answer read surfaces.
-- Existing legacy submit_public_complaint_v2 remains unchanged.
...
-- Admin read contract for configured answers and the exact schema version used.
...
begin
  ...
  if v_complaint.id is null then raise exception 'Complaint not found.' using errcode='P0002'; end if;
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'fieldKey',f.field_key,'labelEn',f.label_en,'labelBn',f.label_bn,
    'fieldType',f.field_type,'storageMode',f.storage_mode,'storageKey',f.storage_key,
    'sortOrder',f.sort_order,'config',f.config,
    'value',case
...
-- Public read contract exposes configured answers only for published reports and
-- only fields not explicitly marked publicVisible=false.
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'fieldKey',f.field_key,'labelEn',f.label_en,'labelBn',f.label_bn,
    'fieldType',f.field_type,'sortOrder',f.sort_order,
