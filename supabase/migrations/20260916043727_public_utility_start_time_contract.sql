-- Complete the published utility timing contract.
-- incident_time is already persisted and required for load-shedding/gas-shortage submissions.
-- Add it to the same sanitized public RPC payloads that now expose utility_end_time.
-- No private reporter context, coordinates, RLS, grants, or submission behavior is changed.
  ...
  SELECT coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'utilityEndTime', c.utility_end_time,
        'utility_end_time', c.utility_end_time
  ...
  SELECT coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'incidentTime', bc.incident_time,
        'incident_time', bc.incident_time,
        ...
        'utilityEndTime', bc.utility_end_time,
        'utility_end_time', bc.utility_end_time
      )
      ORDER BY
