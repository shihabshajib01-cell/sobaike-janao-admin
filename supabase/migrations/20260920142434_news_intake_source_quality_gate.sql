-- Final News Intake source-quality gate.
-- Trusted publishing now requires a fully extracted current incident with
-- substantive source context. Also repairs the one known title-only live report.
...
set description='চট্টগ্রাম নগরীর কোতোয়ালী থানার অদূরে সতীশ বাবু লেইনে এক রিকশাআরোহীকে ছুরি দেখিয়ে তার জিনিসপত্র কেড়ে নেয় দুই ছিনতাইকারী। ঘটনার ভিডিও ছড়িয়ে পড়ার পর পুলিশ জানায়, জড়িতদের ধরতে চেষ্টা চলছে।',
    division='Chattogram',
    district='Chattogram',
    upazila_or_thana='Kotwali',
    area='Patharghata',
    road='Satish Babu Lane',
    formatted_address='সতীশ বাবু লেইন, পাথরঘাটা, কোতোয়ালী, চট্টগ্রাম',
    ...
    publication_preferences=jsonb_set(
      coalesce(c.publication_preferences,'{}'::jsonb),
      '{publicSummaryBn}',
      to_jsonb('চট্টগ্রাম নগরীর কোতোয়ালী থানার অদূরে সতীশ বাবু লেইনে এক রিকশাআরোহীকে ছুরি দেখিয়ে তার জিনিসপত্র কেড়ে নেয় দুই ছিনতাইকারী। ঘটনার ভিডিও ছড়িয়ে পড়ার পর পুলিশ জানায়, জড়িতদের ধরতে চেষ্টা চলছে।'::text),
      true
    ),
    updated_at=now()
where c.id='SJ-2026-729442'
  and c.origin_type='sourced_report'
  and exists (
    select 1 from public.complaint_sources s
    where s.complaint_id=c.id
      and public.normalize_source_url(s.canonical_url)=public.normalize_source_url('https://bangla.bdnews24.com/ctg/vj8mm5sxdq')
  );

insert into public.admin_audit_logs(actor_id,action,target_type,target_id,details)
select auth.uid(),'news_intake.source_grounding_repair','complaint','SJ-2026-729442',
       jsonb_build_object(
         'fields',jsonb_build_array('description','division','district','upazila_or_thana','area','road','formatted_address','sourceOmittedFields'),
         'source_url','https://bangla.bdnews24.com/ctg/vj8mm5sxdq',
         'reason','Verified bdnews24 source contains substantive incident context and a specific Chattogram/Kotwali/Satish Babu Lane location; previous title-only extraction incorrectly marked location as omitted.',
         'timestamp',clock_timestamp()
       )
where exists (
  select 1 from public.complaints c
  where c.id='SJ-2026-729442'
    and c.description<>c.title
    and c.district='Chattogram'
)
and not exists (
  select 1 from public.admin_audit_logs l
  where l.action='news_intake.source_grounding_repair'
    and l.target_id='SJ-2026-729442'
    and l.details->>'source_url'='https://bangla.bdnews24.com/ctg/vj8mm5sxdq'
);
