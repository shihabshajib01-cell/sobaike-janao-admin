-- Extend the persistent public Edge rate limiter to visit-session analytics.
...
alter table public.public_write_rate_events
  ...
  check (action in ('complaint','response','engagement','session'));
...
begin
  ...
  if v_action='session' and (
       v_minute_count >= 20 or v_hour_count >= 120 or v_day_count >= 500
     ) then
    return jsonb_build_object('allowed',false,'retryAfterSeconds',60);
  end if;
